...
router = APIRouter(
    prefix="/management-view/compliance", tags=["Compliance - Management View"]
)
...
async def index(
    ...
    projects = await service.get_all_by_filters({"type_": "HA"})
    min_year = await service.min_year() or datetime.now().year
    return templates.TemplateResponse(
        request,
        "pages/management_view/vapt/index.html",
        {"projects": projects, "min_year": int(min_year), "type_": "Compliance"},
    )
