...
router = APIRouter(prefix="/management-view", tags=["Management View"])
...
async def overall(
    ...
    try:
        project_uuid = UUID(project_id)
        project = await service.get_one_by_id(project_uuid)
        return templates.TemplateResponse(
            request,
            "pages/management_view/vapt/component/overall/index.html",
            {"project": project, "view": view_type},
        )
    except ValueError:
        return templates.TemplateResponse(request, "empty.html")
...
@router.get("/{project_type}", response_class=HTMLResponse)
async def index(
    request: Request,
    service: Annotated[ProjectManagementServiceDep, Depends()],
    project_type: str,
):
    project_type_map = {"vapt": "VA", "compliance": "HA"}
    projects = await service.get_all_by_filters(
        {"type_": project_type_map.get(project_type, "VA")}
    )
